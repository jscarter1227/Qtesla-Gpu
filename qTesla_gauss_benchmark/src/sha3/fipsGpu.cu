...
__device__ __host__ static uint64_t load64(const unsigned char *x)
{
...
__device__ __host__ static void store64(uint8_t *x, uint64_t u)
{
  unsigned int i;
...
__device__ __host__ void KeccakF1600_StatePermute(uint64_t * state)
{
...
__device__ __host__ static void keccak_absorb(uint64_t *s, unsigned int r, const unsigned char *m, unsigned long long int mlen, unsigned char p)
{
...
__device__ __host__ static void keccak_squeezeblocks(unsigned char *h, unsigned long long int nblocks, uint64_t *s, unsigned int r)
{
  unsigned int i;
...
__device__ __host__ void shake128(unsigned char *output, unsigned long long outlen, const unsigned char *input,  unsigned long long inlen)
{
  uint64_t s[25] = {0};
...
__device__ __host__ void cshake128_simple_absorb(uint64_t s[25], uint16_t cstm, const unsigned char *in, unsigned long long inlen)
{
...
__device__ __host__ void cshake128_simple_squeezeblocks(unsigned char *output, unsigned long long nblocks, uint64_t *s)
{
  keccak_squeezeblocks(output, nblocks, s, SHAKE128_RATE);
}
...
__device__ __host__ void shake256_absorb(uint64_t *s, const unsigned char *input, unsigned int inputByteLen)
{
	keccak_absorb(s, SHAKE256_RATE, input, inputByteLen, 0x1F);
}


__device__ __host__ void shake256_squeezeblocks(unsigned char *output, unsigned long long nblocks, uint64_t *s)
{
...
__device__ __host__ void cshake256_simple_absorb(uint64_t s[25], uint16_t cstm, const unsigned char *in, unsigned long long inlen)
{
  unsigned char *sep = (unsigned char*)s;
  unsigned int i;
...
__device__ __host__ void cshake256_simple_squeezeblocks(unsigned char *output, unsigned long long nblocks, uint64_t *s)
{
  keccak_squeezeblocks(output, nblocks, s, SHAKE256_RATE);
}
