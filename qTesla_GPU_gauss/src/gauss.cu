...
__global__ void kmxGaussGpu( int32_t z[/*CHUNK_SIZE*/], const unsigned char *seed, int nonce )
{ // Generate CHUNK_SIZE samples from the normal distribution in constant-time
...
{ // Gaussian sampler
  ...
  cudaError_t errCode=cudaSuccess;
	poly *dev_z;
    const unsigned char *dev_seed;
	//int *dev_nonce;

	//allocate on the device
	errCode=cudaMalloc((poly**)&dev_z, sizeof(int32_t)*PARAM_N);
	if(errCode != cudaSuccess) {
	cout << "\nError: A error with code " << errCode << endl;
	}

	errCode=cudaMalloc((const unsigned char **)&dev_seed, sizeof(unsigned char)*( 4*CRYPTO_SEEDBYTES ));
	if(errCode != cudaSuccess) {
	cout << "\nError: C error with code " << errCode << endl;
	}

	//copy poly to device
	errCode=cudaMemcpy( dev_z, z, sizeof(int32_t)*PARAM_N, cudaMemcpyHostToDevice);
	if(errCode != cudaSuccess) {
	cout << "\nError: A memcpy error with code 1 " << errCode << endl;
	}

	//copy seeds to device
	errCode=cudaMemcpy( (void *)dev_seed, seed, sizeof(unsigned char)*( 4*CRYPTO_SEEDBYTES ), cudaMemcpyHostToDevice);
	if(errCode != cudaSuccess) {
	cout << "\nError: A memcpy error with code 2 " << errCode << endl;
	}

    //execute kernel
	const unsigned int totalBlocks=ceil(CRYPTO_SEEDBYTES*4.0/1024.0);

    for (int chunk = 0; chunk < PARAM_N; chunk += CHUNK_SIZE) {
     
      kmxGaussGpu<<<totalBlocks,1024>>>(*(dev_z + chunk), dev_seed, nonce);

    }

    //copy data from device to host
    errCode=cudaMemcpy( z, dev_z, sizeof(int32_t)*PARAM_N, cudaMemcpyDeviceToHost);
    if(errCode != cudaSuccess) {
