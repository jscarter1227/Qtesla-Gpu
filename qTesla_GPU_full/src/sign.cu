...
__device__ __host__ static __inline int32_t Abs(int32_t value)
...
__device__ __host__ static int test_rejection(poly z)
...
__device__ __host__ static int test_correctness(poly v)
...
__device__ __host__ static int test_z(poly z)
...
__device__ __host__ static int check_ES(poly p, unsigned int bound)
...
**********************************************************/
__global__ void crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
  unsigned char randomness[CRYPTO_RANDOMBYTES], randomness_extended[4*CRYPTO_SEEDBYTES];
  poly a, t;
  poly *e = (poly *)malloc( sizeof(int32_t) * PARAM_N);
  poly *s = (poly *)malloc( sizeof(int32_t) * PARAM_N);
