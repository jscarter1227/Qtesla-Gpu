...
int main(void)
{
  ...
  unsigned char *pk_dev;
  unsigned char *sk_dev;
  ...
  for (i = 0; i < NRUNS; i++) {
    ...
    errCode=cudaMalloc((unsigned char**)&pk_dev, sizeof(unsigned char)*CRYPTO_PUBLICKEYBYTES);
    if(errCode != cudaSuccess) {
    cout << "\nError: A error with code " << errCode << endl;
    }

    errCode=cudaMalloc((unsigned char**)&sk_dev, sizeof(unsigned char)*CRYPTO_SECRETKEYBYTES);
    if(errCode != cudaSuccess) {
    cout << "\nError: A error with code " << errCode << endl;
    }

    errCode=cudaMemcpy( pk_dev, pk, sizeof(unsigned char)*CRYPTO_PUBLICKEYBYTES, cudaMemcpyHostToDevice);
    if(errCode != cudaSuccess) {
    cout << "\nError: A memcpy error with code " << errCode << endl;
    }

    errCode=cudaMemcpy( sk_dev, sk, sizeof(unsigned char)*CRYPTO_SECRETKEYBYTES, cudaMemcpyHostToDevice);
    if(errCode != cudaSuccess) {
    cout << "\nError: A memcpy error with code " << errCode << endl;
    }
    ...
    cycles0[i] = cpucycles();
    crypto_sign_keypair<<<totalBlocks,1024>>>(pk_dev, sk_dev);
    cycles0[i] = cpucycles() - cycles0[i];

    //copy data from device to host
    errCode=cudaMemcpy( pk, pk_dev sizeof(unsigned char)*CRYPTO_PUBLICKEYBYTES, cudaMemcpyDeviceToHost);
    if(errCode != cudaSuccess) {
    cout << "\nError: getting C result form GPU error with code " << errCode << endl;
    }

    errCode=cudaMemcpy( sk, sk_dev, sizeof(unsigned char)*CRYPTO_SECRETKEYBYTES, cudaMemcpyDeviceToHost);
    if(errCode != cudaSuccess) {
    cout << "\nError: getting C result form GPU error with code " << errCode << endl;
    }
