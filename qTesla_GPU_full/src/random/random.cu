...
__device__ __host__ static __inline void delay(unsigned int count)
{
  while (count--) {}
}


__device__ __host__ static int randombytes_internal(unsigned char* random_array, unsigned int nbytes)
...
__device__ __host__ void randombytes(unsigned char* random_array, unsigned int nbytes)
{ // Generation of "nbytes" of random values
